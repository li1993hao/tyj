...
<div class="main-container" id="main-container">
    ...
    <div class="btn-group">
        <a class="btn btn-sm btn-primary ajax-post" href="<?php echo U('sports?id='.$id);?>" target-form="code">保存</a>
        <?php if($method == 'checked'): ?><a class="btn btn-sm btn-primary" href="<?php echo U('sports?id='.$id);?>" >显示所有</a>
        <?php else: ?>
            <a class="btn btn-sm btn-primary" href="<?php echo U('sports?method=checked&id='.$id);?>" >只显示已选</a><?php endif; ?>
        <a class="btn btn-sm" href="<?php echo U('manager');?>">返回</a>
    </div>
     <div class="table-responsive">
        <table class="table table-striped  ">
            ...
            <tbody>
            <?php if(!empty($nodeList)): if(is_array($nodeList)): $i = 0; $__LIST__ = $nodeList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$node): $mod = ($i % 2 );++$i;?><tr class="level" <?php if(($node["level"]) != "0"): ?>style="display:none"<?php endif; ?> data-level="<?php echo ($node['level']); ?>" >
                        <td class="center">
                            <?php if(!isset($node["children"])): ?><input type="checkbox" value="<?php echo ($node["code"]); ?>" name="code[]" <?php if(in_array($node['code'],$data)): ?>checked<?php endif; ?> class="ace ids code">
                                <span class="lbl"></span>
                             <?php else: ?>
                                <input type="checkbox" class="ace ids">
                                <span class="lbl"></span><?php endif; ?>
                        </td>
                        <td>
                            <?php if(isset($node["children"])): ?><div style="display: inline-block" class=" node-folder" data-status='close'><i class="icon-plus smaller-100"></i></div><?php endif; ?>
                            <?php $__FOR_START_1191198146__=0;$__FOR_END_1191198146__=$node["level"];for($i=$__FOR_START_1191198146__;$i < $__FOR_END_1191198146__;$i+=1){ if($i == $node['level']-1): if($node['last']): ?>|__
                                        <?php else: ?>
                                        |--<?php endif; ?>
                                    <?php else: ?>
                                    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<?php endif; } ?>
