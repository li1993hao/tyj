    ...
    <form action="<?php echo U();?>" method="post" class="form-horizontal normal-form">
        <div class="form-group">
            ...
            <select id="com_id" name="com_id">
                <?php if(is_array($com)): $i = 0; $__LIST__ = $com;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i; if($info['com_id'] == $vo['id']): ?><option selected value="<?php echo ($vo["id"]); ?>" ><?php echo ($vo["name"]); ?></option>
                        <?php else: ?>
        ...
        <div class="form-group ">
            ...
            <div style="display: inline-block" class="add_sport">
                <?php if(isset($info)): ?><input type="hidden" name="sports" value="<?php echo ($info["sports"]); ?>"/><label><?php echo (get_sports($info["sports"],'cp_name')); ?></label><?php endif; ?>
            </div>
            <a class="btn btn-sm btn-primary" onclick="addSports();" href="javascript:;">选择</a>
        </div>
        <div class="form-group ">
            <label class="item-label">时间:</label>
                <input type="text" class="date_3" <?php if(isset($info)): ?>value="<?php echo (date('Y-d',$info["time"])); ?>"<?php endif; ?>  name="time"/>
        </div>
        <div class="form-group">
            <label class="item-label">地点:</label>
            <input name="place" type="text" value="<?php echo ($info["place"]); ?>" maxlength="100"/>
        </div>
        <div class="form-group">
            <label class="item-label">名次:</label>
            <select name="ranking">
                <?php if(is_array(C("COM_MINGCI"))): $i = 0; $__LIST__ = C("COM_MINGCI");if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$value): $mod = ($i % 2 );++$i;?><option <?php if(($info["ranking"]) == $key): ?>selected<?php endif; ?> value="<?php echo ($key); ?>"><?php echo ($value); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
            </select>
        </div>
        <div class="form-group">
            <label class="item-label">奖牌:</label>
            <select name="medal">
                <?php if(is_array(C("COM_jiangpai"))): $i = 0; $__LIST__ = C("COM_jiangpai");if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$value): $mod = ($i % 2 );++$i;?><option <?php if(($info["medal"]) == $key): ?>selected<?php endif; ?>  value="<?php echo ($key); ?>"><?php echo ($value); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
            </select>
        </div>
        <div class="form-group">
            <label class="item-label">备注:</label>
            <textarea name="remark" maxlength="255"><?php echo ($info["remark"]); ?></textarea>
        </div>
        <div class="form-group">
...
<script type="text/javascript" src="/tyj/Public/static/js/common.js"></script>
    ...
    <script type="text/javascript">
        function addSports(){
            winopen("<?php echo U('tool/selectSports?method=uid');?>",500,600);
        }
