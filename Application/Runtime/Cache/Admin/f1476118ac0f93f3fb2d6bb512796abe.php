        ...
        <div class="btn-group">
            ...
            <ul class="dropdown-menu dropdown-primary">
                <?php if(isset($current_type)): ?><li>
                        <a href="<?php echo U('planManager');?>" >全部</a>
                    </li>
                    <?php if(is_array($types)): foreach($types as $key=>$vo): ?><li>
                            <?php if(($current_type) != $key): ?><a href="<?php echo U('planManager?type='.$key);?>"><?php echo ($vo); ?></a><?php endif; ?>
                        </li><?php endforeach; endif; ?>
                    <?php else: ?>
                    <?php if(is_array($types)): foreach($types as $key=>$vo): ?><li>
                            <a href="<?php echo U('planManager?type='.$key);?>"><?php echo ($vo); ?></a>
    ...
    <div class="table-responsive">
        <table class="table table-striped table-bordered table-hover">
            ...
            <?php if(!empty($list_data)): if(is_array($list_data)): $i = 0; $__LIST__ = $list_data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                        ...
                        <td>
                            <?php if($vo['status'] == 2): ?><a  href="<?php echo U('planVerify?id='.$vo['id']);?>">审核</a>
                            <?php else: ?>
                                <a  href="<?php echo U('planVerify?id='.$vo['id']);?>">详细</a><?php endif; ?>
                        </td>
