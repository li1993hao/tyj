...
<div class="main-container" id="main-container">
    ...
    <div>
        <div class="btn-group">
            ...
            <button class="btn btn-sm btn-primary ajax-post confirm" url="<?php echo U('setStatus?status=-1');?>" target-form="ids">删 除</button>
        </div>
        <!-- 高级搜索 -->
    ...
    <div class="table-responsive">
        <table class="table table-striped table-bordered table-hover">
            ...
            <?php if(!empty($list)): if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$com): $mod = ($i % 2 );++$i;?><tr>
                        ...
                        <td><?php echo CC('COM_QUYU',$com['quyu']);?></td>
                        <td><?php echo CC('COM_JIJIE',$com['jijie']);?></td>
                        <td><?php echo CC('COM_NIANLIN',$com['nianlin']);?></td>
                        <td><?php echo CC('COM_MINGCHENG',$com['mingcheng']);?></td>
                        <td>
                            <?php echo ($com["status_text"]); ?>
                        </td>
                        <td>
                            <?php if(($com["status"]) == "1"): ?><a href="<?php echo U('setStatus?status=0&ids='.$com['id']);?>" class="ajax-get">禁用</a>
                                <?php else: ?>
                                <a href="<?php echo U('setStatus?status=1&ids='.$com['id']);?>" class="ajax-get">启用</a><?php endif; ?>
