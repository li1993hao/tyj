...
<div class="main-container" id="main-container">
   ...
   <?php echo ($meta_title); ?>
    ...
    <form class="form-horizontal " action="<?php echo U(ACTION_NAME.'?model='.$model['id']);?>" method="post">
        <!-- 标签页导航 -->
<div class="tabbable">
            ...
            <?php if(is_array($fields[$key])): $i = 0; $__LIST__ = $fields[$key];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$field): $mod = ($i % 2 );++$i; if($field['is_show'] == 1 || $field['is_show'] == 3): ?><div class="form-group">
                        ...
                        <div class="controls">
                            <?php switch($field["type"]): case "num": ?><input type="text" class="text input-mid" name="<?php echo ($field["name"]); ?>"
                                ...
                                <?php case "date_view_4": ?><input type="text" name="<?php echo ($field["name"]); ?>" class="text input-mid date_view_4" value="<?php echo (date('Y-m-d',$data[$field['name']])); ?>" placeholder="请选择日期" /><?php break;?>
                                <?php case "date_3": ?><input type="text" name="<?php echo ($field["name"]); ?>" class="text input-mid date_3" value="<?php echo (date('Y-m',$data[$field['name']])); ?>" placeholder="请选择日期" /><?php break;?>
