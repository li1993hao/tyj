...
<div class="main-container" id="main-container">
    ...
    <div>
    <a class="btn btn-sm btn-primary" data-toggle="modal" href="#addModal">添加</a>
     </div>
    ...
    <table class="table table-striped table-bordered table-hover">
        ...
        <tbody>
        <?php if(empty($model)): ?><td colspan="10" class="text-center"> aOh! 暂时还没有内容! </td>
            <?php else: ?>
            <?php if(is_array($model)): $i = 0; $__LIST__ = $model;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$m): $mod = ($i % 2 );++$i;?><tr>
                    <td><?php echo ($m["name"]); ?></td>
                    <td><?php echo ($m["description"]); ?></td>
                    <td><?php echo ($m["category"]); ?></td>
                    <td><?php echo ($m["count"]); ?></td>
                    <td><a  class="ajax-get confirm" href="<?php echo U('delete?id='.$m['id']);?>">删除</a>|<a
                            class="ajax-get confirm" href="<?php echo U('clear?id='.$m['id']);?>">清空</a></td>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title text-center" id="login-label">添加模型</h4>
                </div>
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        ...
                        <label>
                            <i  class="icon-folder-close" style="display: none"></i>
                            <span class="upload-file-item" style="display: none">
                             </span>
                        </label>
                    </div>
                </div>
...
<script src="/tyj/Public/static/js/common.js"></script>
    ...
    <script type="text/javascript" src="/tyj/Public/vender/uploadify/jquery.uploadify.min.js"></script>
    <script type="text/javascript">
        //上传文件
        /* 初始化上传插件 */
        $("#upload_file").uploadify({
            "height": 30,
            "swf": "/tyj/Public/vender/uploadify/uploadify.swf",
        ...
        function uploadFile(file, data) {
            ...
            if (json.status) {
                setTimeout(function () {
                    $(".icon-folder-close").show();
                    $(".upload-file-item").show().text(json.msg);
                }, 1500);
                $("#file").val(json.data);
            } else {
                errorAlert(json.msg);
        ...
        function add() {
            ...
            var file = $("#file").val();
            if (name == "") {
                $("#error").empty().text("名称不能为空!");
                return;
            }
            if (des == "") {
                $("#error").empty().text("描述不能为空!");
                return;
            }

            if (file == "") {
                $("#error").empty().text("请上传文件!");
                return;
            }
            showLoading();
            $.post(
                    "<?php echo U(add);?>", {"name": name, "des": des, "file": file}, function (data) {
                        removeLoading();
                        if (data.status) {
                            okAlert(data.msg);
                            setTimeout(function(){
