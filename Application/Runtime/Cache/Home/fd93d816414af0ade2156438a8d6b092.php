...
<div class="main-container" id="main-container">
    ...
    <div class="btn-group">
        <a class="btn btn-sm btn-primary" href="<?php echo U('add?type=1');?>">添加栏目</a>
    ...
    <div class="able-responsive">
        ...
        <?php if(!empty($nodeList)): if(is_array($nodeList)): $i = 0; $__LIST__ = $nodeList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$node): $mod = ($i % 2 );++$i;?><tr>
                    <td><?php echo ($node["priority"]); ?></td>
                    <td><?php echo ($node["id"]); ?></td>
                    <td>
                        <?php $__FOR_START_1922198793__=0;$__FOR_END_1922198793__=$node["level"];for($i=$__FOR_START_1922198793__;$i < $__FOR_END_1922198793__;$i+=1){ if($i == $node['level']-1): if($node['last']): ?>|__
                                    <?php else: ?>
                                    |--<?php endif; ?>
                                <?php else: ?>
                                &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<?php endif; } ?>
                        <?php echo ($node["name"]); ?></td>
                    <td><?php switch($node["type"]): case "1": ?>栏目<?php break;?>
                    ...
                    <td><?php switch($node["type"]): case "1": ?><a href="<?php echo U('delete?id='.$node['id']);?>" class="ajax-get confirm">删除</a>|<a href="<?php echo U('edit?id='.$node['id'].'&type=1');?>">修改</a>|<a href="<?php echo U('add?select_id='.$node['id']);?>">添加子栏目</a><?php break;?>
                        <?php case "2": ?><a href="<?php echo U('delete?id='.$node['id']);?>" class="ajax-get confirm">删除</a>|<a href="<?php echo U('edit?id='.$node['id'].'&type=2');?>">修改</a><?php break;?>
                        <?php case "3": ?><a href="<?php echo U('delete?id='.$node['id']);?>" class="ajax-get confirm">删除</a>|<a href="<?php echo U('edit?id='.$node['id'].'&type=3');?>">修改</a><?php break;?>
                        <?php default: ?>栏目<?php endswitch;?></td>
