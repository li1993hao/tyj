...
class AuthGroupModel extends Model {
    ...
    const AUTH_EXTEND_CATEGORY_TYPE = 1;              // 栏目管理权限
    ...
    public function addToGroup($uid,$gid){
        $uid = !is_array($uid)?str2arr($uid):$uid;
        $gid = !is_array($gid)?str2arr($gid):$gid;

        $Access = M(self::AUTH_GROUP_ACCESS);
        if( isset($_REQUEST['batch']) ){
            //为用户批量添加用户组时,先删除旧数据
            $del = $Access->where( array('uid'=>array('in',arr2str($uid))) )->delete();
        }

        $add = array();
        if( $del!==false && $gid[0]>0){
            foreach ($uid as $u){
                foreach ($gid as $g){
    ...
    static public function memberInGroup($group_id){
        ...
        $list     = M() ->field('m.id,m.username,m.last_login_time,m.last_login_ip,status')
                       ->table($l_table.' m')
    ...
    public function checkCategoryId($cid){
        return $this->checkId('Node',$cid, '以下分类id不存在:');
    }
}
