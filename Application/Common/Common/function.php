 ...
 * @return array
 */
function list_to_tree($list, $pk, $pid, $child_key ='children', $root = 0){
    $tree = array();
    if(is_array($list)){
        //主键引用
        $refer = array();
 ...
 */
function tree_to_list_first($tree,$child_key,&$list=array(),$level=0){
    if(is_array($tree)){
...
function list_url($list){
    if(!isset($list['id'])){
        foreach($list as $k=>$v){
            $list[$k]['url'] = U('Home/Index/category?id='.$v['id'].'&p=1');
        }
    }else{
        $list['url']= U('Home/Index/category?id='.$list['id'].'&p=1');
    }
