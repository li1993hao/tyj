...
class DocumentApi {
     ...
     * @return array              文档列表
     */
    public static function lists($category,$page=1,$order = '`is_up` DESC, `update_time` DESC', $status = 1){
        ...
        $model = M($model_name)->field('content',true)->where($map)->order($order);
        if(is_numeric($page)){
    ...
    public  static function  search($search,$page,$page_num,$order = '`is_up` DESC, `update_time` DESC',$status=1){
        $map = DocumentApi::listMap(null, $status);
        $search_sql = arr2str($search,'%');
        $map[] ="BINARY `title` LIKE '%{$search_sql}%'";
        $model = M('article')->field('content',true)->where($map)->order($order);
        if(is_numeric($page)){
    ...
    public  static function  searchCount($search,$order = '`is_up` DESC, `update_time` DESC',$status=1){
        $map = DocumentApi::listMap(null, $status);
        $search_sql = arr2str($search,'%');
        $map[] ="BINARY `title` LIKE '%{$search_sql}%'";
