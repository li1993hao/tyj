...
class CategoryApi {
    ...
    public static function get_category($id, $field = null){
        ...
        /* 获取分类名称 */
        if(!isset($list[$id])){
            $cate = M('node')->where(array('status'=>1,'type'=>array('in','1,2,3')))->find($id);
    ...
    public static function get_children_category($cid){
        ...
        $result =  M('node')->where(array('status'=>1,'pid'=>$cid,'type'=>array('in','1,2,3')))->order('sort')->select();
    ...
    public static function get_children_count($cid,$map=array()){
        ...
        $map = array_merge(array('pid'=>$cid,'status'=>1,'type'=>array('in','1,2,3')),$map);
