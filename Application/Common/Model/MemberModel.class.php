...
class MemberModel extends \Think\Model{
    ...
    protected $_auto = array(
        array('password', 'user_encrypt', self::MODEL_BOTH, 'function', UC_AUTH_KEY),
     ...
     */
    public function register($username, $password,$nickname,$type,$status=0, $email, $mobile){
        $data = array(
            'username' => $username,
            'nickname' =>$nickname,
            'password' => $password,
     ...
     * @param  $checkType  是否进行管理权限认证 用于后台等的时候验证
     * @return integer           登录成功-用户ID，登录失败-错误编号
     */
    public  function  checkLogin($username, $password, $type = 1, $checkType=true){
        $map = array();
        switch ($type) {
            case 1:
